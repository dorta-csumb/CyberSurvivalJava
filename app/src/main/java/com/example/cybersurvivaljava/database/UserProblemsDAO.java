...
public interface UserProblemsDAO {
    ...
    // 3. Speed = 0 (Data missing in current schema, placeholder for now).
    ...
    @Query("SELECT " +
            ...
            "0 AS totalSpeed, " +
            "CAST(SUM(CASE WHEN up.isCorrect THEN 1 ELSE 0 END) AS INTEGER) AS tasksCompleted " + // <--- Added CAST here
